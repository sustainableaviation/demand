...
for icao_departure in departure_airports_geodf['icao'][:]:
    ...
    file_path = os.path.join(current_directory, f"Airport Data/01-January/{icao_departure}.json")
    try:
        with open(file_path, 'r') as f:
            airport_connections = json.load(f)
    except FileNotFoundError:
        print(f"Warning: JSON file for {icao_departure} during not found. Skipping this airport.")
        continue
