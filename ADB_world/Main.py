...
month_list = ["01-January", "02-February", "03-March", "04-April", "05-May", "06-June", "07-July", "08-August", "09-September", "10-October", "11-November", "12-December"]
# Function to generate flight connections data for each month
...
for month in month_list:
    all_connections, sum_connections = def_flight_connection.process_flight_connections(month)
...
def generate_monthly_figure():
    ...
    max_opacity = 1
    min_opacity = 0.0   
    global_min_opacity = min([monthly_connections[month]["all_connections"]['daily_flights'].min() for month in month_list])
    global_max_opacity = max([monthly_connections[month]["all_connections"]['daily_flights'].max() for month in month_list])

    
    # Create traces for each month
    for month_data in month_list:
        # Calculate the maximum value of 'daily_flights' column for scaling
        max_daily_flights = monthly_connections[month_data]["sum_connections"]['number_of_total_flights'].max()

        # Logarithmically scale the values between min and max sizes
        ...
        fig.add_trace(go.Scattergeo(
            ...
            text=monthly_connections[month_data]["sum_connections"]["departure_name"] + '<br>' +
        ...
        scaled_opacities = ((monthly_connections[month_data]["all_connections"]['daily_flights'] - global_min_opacity) /
                        (global_max_opacity - global_min_opacity) *
