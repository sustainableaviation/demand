...
def reset_inputs(event):
    global route_inputs, final_df, icao
...
def add_route(event):
    global route_input, final_df, line_graph_pane, line_fig
    ...
    def update_final_df(dep_value, dest_value):
        ...
        if dep_value and dest_value:  # Ensure both inputs have values
            # Check if new_df is filled only with zeros (excluding the 'Year' column)
            if not new_df.drop(columns='Year').eq(0).all().all():
                ...
                # Update the line graph pane with the new final_df
                # Clear existing traces before adding new ones
                ...
                line_fig.update_layout(
                    legend=dict(
                        ...
                        yanchor="middle",  # Anchor legend to the middle vertically
                        y=-0.2,  # Positioning of the legend in the middle
                        xanchor="center",  # Anchor legend to the center horizontally
                        x=0.5  # Positioning of the legend in the center
                    )
                )
...
route_view_layout[11:12, 0:12] = pn.Spacer()
