...
# Load the yearly seats per airport data
with open('data/yearly_seats_per_airport.json') as json_file:
    yearly_seats_per_airport = json.load(json_file)
...
# Function to get the initial departing PAX for the given ICAO codes
def get_initial_departing_pax(icao_codes):
    total_pax = 0
    for icao_code in icao_codes:
        if icao_code in yearly_seats_per_airport:
            total_pax += yearly_seats_per_airport[icao_code]
    return (total_pax * 0.8)
...
def display_icao_codes(event):
    ...
    if country_name in country_coord_df.index:
        ...
        # Update dataframe and line graph based on selected country
        initial_departing_pax = get_initial_departing_pax(matching_icao_codes)
        update_country_comparison_pane(initial_departing_pax, matching_icao_codes)
    else:
...
def update_country_comparison_pane(initial_departing_pax, matching_icao_codes):
    # Create the dataframe for the country comparison
    years = list(range(2024, 2051))
    departing_pax = [initial_departing_pax]
    scaling_factors = get_scaling_factors(matching_icao_codes[0])
    scaling_factors.append(scaling_factors[-1])  # Extend the scaling factors to match the years length

    # Extend the scaling factors to match the length of years
    if len(scaling_factors) < len(years):
        scaling_factors.extend([scaling_factors[-1]] * (len(years) - len(scaling_factors)))

    # Convert scaling factors to percentages
    scaling_factors_percent = [f"{factor * 200}%" for factor in scaling_factors]

    for i in range(1, len(years)):
        next_pax = departing_pax[-1] * (1 + float(scaling_factors[i - 1])*2)
        departing_pax.append(round(next_pax, 2))

    country_df = pd.DataFrame({
        'Year': years,
        'Departing PAX': departing_pax,
        'Scaling Factor': scaling_factors_percent[:len(years)]
    })
    
    # Create the line graph
    line_fig_country = px.line(country_df, x='Year', y='Departing PAX', markers=True)
    line_graph_pane_country = pn.pane.Plotly(line_fig_country, margin=0, height=700)
    
    # Create the dataframe pane
    dataframe_pane_country = pn.pane.DataFrame(country_df, sizing_mode="stretch_both", max_height=700, index=False)
    
    # Add the new panes to the "Country Comparison" page
    pages["Country Comparison"][2:7, 0:10] = comparison_map
    pages["Country Comparison"][7:10, 0:5] = dataframe_pane_country
    pages["Country Comparison"][7:10, 5:10] = line_graph_pane_country
...
def show(page):
    if page == "World View":
        ...
        pages[page][2:7, 0:10] = comparison_map
        pages[page][7:10, 0:10] = pn.Spacer()
        pages[page][10:13, 0:10] = pn.Spacer()
    return pages[page]
